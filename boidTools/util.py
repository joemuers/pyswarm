...
######################################            
def ScriptNodeExists(scriptNodeName, isBefore):
    kwargs = { "query" : True }
    if(isBefore):
...
##################################################\n\n" % PackageName())
    ...
    if(not ScriptNodeExists("pySwarmOnSceneOpen", True)):
...
__import__(\"%s\", globals(), locals(), [\"%s\"], -1)" % 
    ...
    if(not ScriptNodeExists("pySwarmOnFrameUpdate", True)):        
    ...
    if(not ScriptNodeExists("pySwarmOnSceneClose", False)): 
