...
##########################    
    def _restrictTurnrateIfNecessary(self, agent, desiredAcceleration, maxTurnrate):
        currentVelocity = agent.currentVelocity
        potentialVelocity = currentVelocity + desiredAcceleration
        madeChanges = False
        
        angleChange = potentialVelocity.angleFrom(currentVelocity)
        if(angleChange > maxTurnrate):
            potentialVelocity.rotateInHorizontal(angleChange - maxTurnrate)
            potentialVelocity.subtract(currentVelocity)
            desiredAcceleration.resetVec(potentialVelocity)
            madeChanges = True
        elif(angleChange < -maxTurnrate):
            potentialVelocity.rotateInHorizontal(angleChange + maxTurnrate)
            potentialVelocity.subtract(currentVelocity)
            desiredAcceleration.resetVec(potentialVelocity)
            madeChanges = True
